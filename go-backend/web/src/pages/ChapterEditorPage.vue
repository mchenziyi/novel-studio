...
// ====== SCROLL SYNC ======
function syncScroll(src:HTMLElement){
  if(vSyncing)return;vSyncing=true
  if(leftGut.value&&src!==leftGut.value)leftGut.value.scrollTop=src.scrollTop
  if(leftCon.value&&src!==leftCon.value){leftCon.value.scrollTop=src.scrollTop;leftCon.value.scrollLeft=src.scrollLeft}
  if(edtGut.value&&src!==edtGut.value)edtGut.value.scrollTop=src.scrollTop
  if(edtScr.value&&src!==edtScr.value){edtScr.value.scrollTop=src.scrollTop;edtScr.value.scrollLeft=src.scrollLeft}
  requestAnimationFrame(()=>{vSyncing=false})
}
function hs(e:Event){syncScroll(e.target as HTMLElement)}
function vs(e:Event){syncScroll(e.target as HTMLElement)}
    ...
    <div class="flex-1 flex min-h-0 bg-white relative">
      ...
      <div :style="{width:leftW+'%'}" class="border-r border-[#e5e5e5] flex flex-col shrink-0">
        ...
        <div v-if="selectedVer" class="flex-1 flex min-h-0">
          <div ref="leftGut" class="shrink-0 w-[44px] bg-[#fcfcfc] border-r border-[#f0f0f0] overflow-hidden select-none" @scroll="vs">
          ...
          <div ref="leftCon" class="flex-1 overflow-auto" @scroll="hs">
      ...
      <div :style="{width:midW+'%'}" class="flex flex-col shrink-0 relative">
        ...
        <div class="flex-1 flex min-h-0" style="font-family:'Georgia','Noto Serif SC',serif">
          <div ref="edtGut" class="shrink-0 w-[44px] bg-[#fcfcfc] border-r border-[#f0f0f0] overflow-hidden select-none" @scroll="vs">
            <div class="text-right pr-1.5 pt-4 text-[11px] leading-6 text-[#ccc]">
              <div v-for="(_,i) in editorLines" :key="i" class="h-6">{{i+1}}</div>
            </div>
          </div>
          <div ref="edtScr" class="flex-1 overflow-auto" @scroll="hs">
